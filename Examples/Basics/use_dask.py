# -*- coding: utf-8 -*-
"""
Example on how to use Dask with TFLearn. Dask is simple task scheduling
...
compatibility demonstration. In practice, there is no so much need to uses
...
network = conv_2d(network, 32, activation='relu')
network = max_pool_2d(network, 2)
network = dropout(network, 0.75)
network = conv_2d(network, 64, 3, activation='relu')
network = conv_2d(network, 64, 3, activation='relu')
network = max_pool_2d(network, 32)
